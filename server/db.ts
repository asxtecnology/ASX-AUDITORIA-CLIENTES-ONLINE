...
/** Resets the DB singleton so the next call to getDb() creates a fresh connection */
...
export async function getActiveProducts(): Promise<Product[]> {
  ...
  if (!db) return [];
  try {
    return await db.select().from(products).where(eq(products.ativo, true)).orderBy(products.codigo);
  } catch (e) {
    console.error("[DB] getActiveProducts error:", e);
    resetDb();
    return [];
  }
}
...
export async function getMonitoringRuns(limit = 20) {
  ...
  if (!db) return [];
  try {
    return await db.select().from(monitoringRuns).orderBy(desc(monitoringRuns.startedAt)).limit(limit);
  } catch (e) {
    console.error("[DB] getMonitoringRuns error:", e);
    resetDb();
    return [];
  }
}
...
export async function getLatestMonitoringRun() {
  ...
  try {
    ...
    console.error("[DB] getLatestMonitoringRun error:", e);
    resetDb();
    return null;
  }
}
...
export async function getViolations(opts?: {
  ...
  try {
    ...
    console.error("[DB] getViolations error:", e);
    resetDb();
    return { items: [], total: 0 };
  }
}
...
export async function getViolationStats() {
  ...
  try {
    ...
    console.error("[DB] getViolationStats error:", e);
    resetDb();
    return { total: 0, open: 0, notified: 0, resolved: 0, todayCount: 0 };
  }
}
